...
function sbmDoctorProcessSiteWideBatchChunk_(o,startIndex,chunkSize){
  ...
  if(!units.length)throw new Error('Site Diagnosis一括結果から登録対象を抽出できませんでした。clusters[] の記事情報を確認してください。');
  ...
  return {
    ...
    message:end<total
      ? ('Site Diagnosis一括結果を分割登録しています。'+end+'/'+total+'件完了、残り'+(total-end)+'件です。')
      : ('Site Diagnosis一括結果 '+total+'件の登録が完了しました。')
  };
}
