...
const SBM_VERSION = '5.10.0-RC8.14';
...
function sbmDoctorRegisterSiteDiagnosisResult(){
  ...
  var html='<!doctype html><html><head><base target="_top"><meta charset="UTF-8"><style>'+
    ...
    '</style></head><body><h2>Site Diagnosisの処置を進める</h2><div class="flow">Site Diagnosis → Doctor診断結果を登録 → Writer紹介状 → Writer処置結果を登録 → モニター</div>'+
    ...
    '<div id="writerRequestStep" class="step hidden"><div class="stepTitle">② Writerへ依頼</div><div class="note">Doctor診断結果からSBMがWriter紹介状を作成しました。記事を確認してから紹介状をSIMS Writerへ渡してください。</div><textarea id="writerRequest" class="writerText" readonly></textarea><div class="actions"><button id="openArticle" class="outline" onclick="openArticleUrl()" disabled>記事を開く</button><button class="primary" onclick="copyWriterRequest()">Writer紹介状をコピー</button></div></div>'+
...
function sbmDoctorSyncImprovementRoutesFromCases_(){
  ...
  cases.forEach(function(c){
    var hid=String(c['改善履歴ID']||'').trim(),route=sbmDoctorRouteFromCase_(c),writerResult=String(c['Writer結果JSON']||'').trim();
    if(!route)return;
