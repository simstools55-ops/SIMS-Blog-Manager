...
function sbmSearchConsoleApiRequest_(property, body) {
  // Product 5.0: Search Console取得エンジンはRC9系の安定動作を優先する。
  // まずAdvanced Serviceが利用できる環境ではSearchConsoleサービスを使う。
  // これによりUrlFetchApp権限未反映環境でも取得できる可能性を高める。
  if (typeof SearchConsole !== 'undefined'
      && SearchConsole.Searchanalytics
      && typeof SearchConsole.Searchanalytics.query === 'function') {
    try {
      return SearchConsole.Searchanalytics.query(body, property) || {};
    } catch (advancedError) {
      // Advanced Service未設定・無効時はRC9と同じUrlFetchApp方式へフォールバックする。
      if (!/not defined|is not defined|service|有効|disabled|not enabled/i.test(String(advancedError))) {
        throw advancedError;
      }
    }
  }
