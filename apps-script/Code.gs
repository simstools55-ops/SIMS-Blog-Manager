...
function sbmDoctorResumeSiteDiagnosisTreatments(){
  try{
    ...
    vals.forEach(function(row){
      ...
      actions.push({route:route,request:pretty,caseId:caseId,articleUrl:url,resume:true,resumeState:state,resumeLabel:label,siteDiagnosisCaseId:sd});
    });
...
function sbmDoctorStoreMergeTreatmentResult_(o){
  ...
  var existing=rec.hm['Merge結果JSON']?String(rec.values[rec.hm['Merge結果JSON']-1]||''):'';var raw=JSON.stringify(o);if(existing&&existing===raw)return {caseId:m.caseId,status:String(rec.values[rec.hm['状態']-1]||''),alreadyRegistered:true};
...
function sbmDoctorSubmitSiteDiagnosisMergeResult(raw){
  try{
    ...
    if(saved.resultStatus&&/COMPLETED|READY|SUCCESS|APPROVED/i.test(saved.resultStatus)&&sbmDoctorMergeNeedsWriter_(m)){
