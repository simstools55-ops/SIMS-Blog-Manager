...
function sbmDoctorSubmitSiteDiagnosisMergeResult(raw){
  try{
    ...
    return {ok:true,message:'Site DiagnosisのMerge処置結果を登録しました。\nCaseID：'+saved.caseId+'\nSite Diagnosis CaseID：'+siteDiagnosisCaseId+'\n状態：'+saved.status+(writerText?'\n\n次はWriterへ統合本文の編集を依頼してください。':''),caseId:saved.caseId,siteDiagnosisCaseId:siteDiagnosisCaseId,status:saved.status,articleUrl:writerReq&&writerReq.article?writerReq.article.url:(rec.hm['記事URL']?String(rec.values[rec.hm['記事URL']-1]||'').trim():''),writerReady:!!writerText,writerRequest:writerText,articleTitle:writerReq&&writerReq.article?String(writerReq.article.title||''):''};
  }catch(e){return {ok:false,message:String(e&&e.message?e.message:e)};}
}
