...
function sbmDoctorRebuildSiteDiagnosisReferral(caseId,route){
  try{
    ...
    return {ok:true,route:route,request:JSON.stringify(req,null,2),caseId:String(caseId),articleUrl:route==='WRITER'&&req.article?req.article.url:articleUrl};
  }catch(e){return {ok:false,message:String(e&&e.message?e.message:e)};}
}
...
function sbmDoctorResumeSiteDiagnosisTreatments(){
  try{
    ...
    var vals=sh.getRange(2,1,last-1,sh.getLastColumn()).getValues(),actions=[],pendingUser=0;
    vals.forEach(function(row){
      var sd=hm['SiteDiagnosisCaseID']?String(row[hm['SiteDiagnosisCaseID']-1]||'').trim():'';
      if(!sd)return;
      var state=hm['状態コード']?String(row[hm['状態コード']-1]||'').trim():'';
      var caseId=String(row[hm['CaseID']-1]||'').trim(),url=hm['記事URL']?String(row[hm['記事URL']-1]||'').trim():'';
      var route='',req='',label='';
      ...
      actions.push({route:route,request:pretty,caseId:caseId,articleUrl:url,resume:true,resumeState:state,resumeLabel:label,siteDiagnosisCaseId:sd,needsRebuild:needsRebuild});
    });
    if(!actions.length)return {ok:true,actions:[],message:pendingUser?'Site Diagnosis案件は301等の利用者処置待ちです。新しいDoctor結果の登録は不要です。':'再開できるSite Diagnosis処置はありません。'};
    return {ok:true,actions:actions,message:'前回の続きから再開しました。未完了の紹介状／結果登録：'+actions.length+'件'+(pendingUser?'、利用者処置待ち：'+pendingUser+'件':'')};
...
function sbmDoctorRegisterSiteDiagnosisResult(){
  ...
  var html='<!doctype html><html><head><base target="_top"><meta charset="UTF-8"><style>'+
    ...
    '</style></head><body><h2>Site Diagnosisの処置を進める</h2><div class="flow">Site Diagnosis → Doctor → SBM → Writer / Merge → SBM（Mergeは必要時Writerへ再紹介）</div><div id="resumeStatus" class="status"></div>'+
...
function sbmDoctorProcessSiteDiagnosisSingleResult_(o){
  ...
  if(n.mergeReady){
    ...
    return {ok:true,route:'MERGE',mergeReady:true,writerReady:false,message:'Merge紹介状／Package作成済み',request:JSON.stringify(mreq,null,2),mergeRequest:JSON.stringify(mreq,null,2),caseId:id.caseId,siteDiagnosisCaseId:id.siteDiagnosisCaseId,articleUrl:id.articleUrl};
  }
  if(n.writerReady){
    var req=sbmDoctorBuildWriterTreatmentRequest_(source,o,n);
    req.site_diagnosis_context={site_diagnosis_batch_id:id.siteDiagnosisBatchId,site_diagnosis_case_id:id.siteDiagnosisCaseId,case_id:id.caseId};
    sbmDoctorSaveGeneratedWriterRequest_(id.caseId,req);
    return {ok:true,route:'WRITER',writerReady:true,mergeReady:false,message:'Writer紹介状作成済み',request:JSON.stringify(req,null,2),writerRequest:JSON.stringify(req,null,2),caseId:id.caseId,siteDiagnosisCaseId:id.siteDiagnosisCaseId,articleUrl:id.articleUrl};
  }
  if(n.monitor){try{sbmSetArticleWorkStateByIdentity_(id.articleId,id.articleUrl,'👀 モニター中');}catch(ignoreMonitor){}
    return {ok:true,route:'MONITOR',writerReady:false,mergeReady:false,message:'経過観察',caseId:id.caseId,siteDiagnosisCaseId:id.siteDiagnosisCaseId,articleUrl:id.articleUrl};}
  return {ok:true,route:'OTHER',writerReady:false,mergeReady:false,message:saved.label,caseId:id.caseId,siteDiagnosisCaseId:id.siteDiagnosisCaseId,articleUrl:id.articleUrl};
}
function sbmDoctorSubmitSiteDiagnosisResult(rawText){
  try{
    ...
    if(format==='SIMS_DOCTOR_CASE_RESULT_V2'){
      ...
      if(single.request)single.actions=[{route:single.route,request:single.request,caseId:single.caseId,articleUrl:single.articleUrl}];
      return single;
    }
    ...
    units.forEach(function(u){
      try{
        ...
        if(r.request)actions.push({route:r.route,request:r.request,caseId:r.caseId,articleUrl:r.articleUrl});
...
function sbmDoctorNormalizeMergeResult_(o){
  ...
  if(o.envelope&&o.payload){if(String(o.envelope.contract_name||'')!=='SIMS_MERGE_TREATMENT_RESULT_V1')throw new Error('Merge処置結果ではありません。必要なcontract_nameは SIMS_MERGE_TREATMENT_RESULT_V1 です。');return {raw:o,caseId:String(o.payload.case_id||o.envelope.correlation_id||''),requestId:String(o.payload.treatment_request_id||''),resultId:String(o.payload.treatment_result_id||''),status:String(o.payload.result_status||''),decision:o.payload.merge_decision||{},plan:o.payload.merge_plan||{}};}
  if(String(o.format||'')==='SIMS_MERGE_TREATMENT_RESULT_V1')return {raw:o,caseId:String(o.case_id||''),requestId:String(o.treatment_request_id||''),resultId:String(o.treatment_result_id||''),status:String(o.result_status||''),decision:o.merge_decision||{},plan:o.merge_plan||{}};
...
function sbmDoctorSubmitSiteDiagnosisMergeResult(raw){
  try{
    ...
    if((/COMPLETED|READY|SUCCESS|APPROVED/i.test(String(m.status||''))||String(m.status||'')==='')&&sbmDoctorMergeNeedsWriter_(m)){
      writerReq=sbmDoctorBuildWriterRequestFromMergeResult_(m,rec);sbmDoctorSaveMergeWriterRequest_(m.caseId,writerReq);writerText=JSON.stringify(writerReq,null,2);saved.status='Merge統合本文をWriterで処置中';
    }
    return {ok:true,message:'Site DiagnosisのMerge処置結果を登録しました。\nCaseID：'+saved.caseId+'\nSite Diagnosis CaseID：'+siteDiagnosisCaseId+'\n状態：'+saved.status+(writerText?'\n\n次はWriterへ統合本文の編集を依頼してください。':''),caseId:saved.caseId,siteDiagnosisCaseId:siteDiagnosisCaseId,status:saved.status,articleUrl:writerReq&&writerReq.article?writerReq.article.url:(rec.hm['記事URL']?String(rec.values[rec.hm['記事URL']-1]||'').trim():''),writerReady:!!writerText,writerRequest:writerText};
  }catch(e){return {ok:false,message:String(e&&e.message?e.message:e)};}
}
