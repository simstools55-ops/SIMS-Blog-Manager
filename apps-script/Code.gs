/**
 * SIMS-Blog-Manager Product v5.10.5
 * SIMS-Core Slim Edition for blog SEO improvement management.
 * End-user distribution file: paste this entire file into Code.gs/Code.js.
 */

const SBM_VERSION = '5.10.5';
// Product v5.10.5: Site Diagnosis safe-skip and URL/canonical verification guard.
...
function sbmDoctorBuildWriterTreatmentRequest_(sourceRequest,doctor,n){
  ...
  if(!detail.allowed_scope||!detail.allowed_scope.length)throw new Error('Writer紹介状を安全に生成できません。Doctor結果にallowed_scopeがありません。Doctor診断結果の治療範囲を確認してください。');
...
function sbmDoctorSkipSiteDiagnosisTreatment(caseId, reason, detail){
  try{
    ...
    var allowed=['WRITER_IN_PROGRESS','WRITER_REQUEST_READY','MERGE_IN_PROGRESS','MERGE_REQUEST_READY','USER_ACTION_REQUIRED'];
    ...
    return {
      ...
      message:'Site Diagnosis案件を処置せず終了しました。\nCaseID：'+caseId+'\n理由：'+reason+
        (detail?'\n確認メモ：'+detail:'')+
        '\n\nWriter / Mergeの処置結果は登録していません。改善履歴・効果測定も新規作成しません。'
    };
  }catch(e){
