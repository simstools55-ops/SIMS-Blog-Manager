...
## 5.0.0 — Product 5.0 Official — 2026-07-13

- RC3 Performance実機テスト合格版を正式版として確定。
- バージョン表示を `5.0.0` に統一。
- 利用者向けApps Scriptを `コード.gs` 1ファイルに統一。
- GitHub Pagesマニュアルを正式版の画面・操作・4週間効果測定へ更新。
- 過去版テンプレートとRC開発資料を利用者向け配布物から除外。
- 既存RC3 Performanceスプレッドシートを継続利用可能。
