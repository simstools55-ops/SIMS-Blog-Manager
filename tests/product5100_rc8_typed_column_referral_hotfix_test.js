...
ok(build.includes("cand.getRange('A3').setValue('現在地')")&&build.includes("cand.getRange('B3').setValue('精密診断する記事を選ぶ')"),'referral guide is not duplicated across cells');
ok(build.includes("cand.getRange('A4').setValue('次に行うこと')")&&build.includes("cand.getRange('B4').setValue("),'referral next action uses compact horizontal layout');
