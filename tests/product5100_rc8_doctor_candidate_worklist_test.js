...
ok(code.includes("['選択','重症度','記事タイトル','選定理由','状態'"),'selection reason human headers');
ok(code.includes('function sbmDoctorSelectionReason_'),'article-specific selection reason helper');
ok(code.includes("'長期流入低下｜クリック '+cd+'%減（前半→後半）'"),'long-term click decline reason');
ok(code.includes("'長期流入低下｜表示回数 '+id+'%減（前半→後半）'"),'long-term impression decline reason');
