...
must(open.includes('sbmRepairDoctorHistoryDates_()'),'履歴表示時にDoctor旧履歴の日付を軽量補完');
...
for(const h of ["'1週':68","'2週':68","'3週':68","'4週':68","'最終判定':92","'状態':82"])must(polish.includes(h),'週次/状態列 '+h);
must(polish.includes("var visible=['選択','改善日','記事タイトル','改善概要','改善経路','1週','2週','3週','4週','最終判定','状態'];"),'2〜4週を表示');
must(!polish.includes('.autoResizeRows('),'履歴を開く際に行高自動計算をしない');
const repair=fn('sbmRepairDoctorHistoryDates_');
