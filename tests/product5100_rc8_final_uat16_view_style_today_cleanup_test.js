...
must(eff.includes('sbmStyleEffectSheetV2_'),'改善の推移は表示時に装飾を適用');
...
must(hist.includes('sbmApplyHistoryFinalStyle_'),'改善履歴は表示時に装飾を適用');
...
must(openToday.includes('sbmCleanupTodayCompletedRows_()'),'今日の改善を開く時にも完了済み行を掃除');
must(openToday.includes('sbmGetTodayDisplayedRowCount_() < desired'),'空き枠を設定件数まで補充');
