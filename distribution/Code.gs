...
function sbmOpenDailyUpdateDialog() {
  ...
  if (activeHealth && activeHealth.healthCheckId && activeHealth.statusCode !== 'COMPLETED') {
  ...
  var html = '<!DOCTYPE html><html><head><base target="_top"><style>'
    ...
    + 'function startDaily(){terminal=false;el("runBtn").disabled=true;el("runBtn").style.display="none";el("cancelBtn").style.display="none";showRunning("STEP 1 / 3　Search Consoleデータ取得中","Search Consoleから最新のページデータを取得しています。");google.script.run.withFailureHandler(function(e){showFailure("Search Consoleデータの取得に失敗しました。",e,true);}).withSuccessHandler(function(fetch){if(terminal)return;var t=(fetch&&fetch.timing)||{};var detail="STEP1完了 "+formatTime(fetch&&fetch.elapsedSeconds)+"｜API "+formatTime(t.api)+"｜URL正規化 "+formatTime(t.normalize)+"｜状態参照 "+formatTime(t.statusMap)+"｜記事分類 "+formatTime(t.buildRows)+"｜ソート "+formatTime(t.sort)+"｜保存 "+formatTime(t.workWrite)+"。<br>取得したデータを分析し、記事DBと記事ランクを更新しています。";showRunning("STEP 2 / 3　データ分析・記事DB更新中",detail);google.script.run.withFailureHandler(function(e){showFailure("データ分析・記事DB更新に失敗しました。",e,true);}).withSuccessHandler(function(){if(terminal)return;showRunning("STEP 3 / 3　改善の推移・完了処理中","改善後の推移を更新し、日次処理の完了状態を確定しています。");google.script.run.withFailureHandler(function(e){showFailure("改善の推移・完了処理に失敗しました。",e,true);}).withSuccessHandler(function(r){showComplete(r||{});}).sbmRunDailyFinalizeStageFromDialog();}).sbmRunDailyAnalysisStageFromDialog();}).sbmRunDailyFetchStageFromDialog();}'
    + 'function showComplete(r){if(terminal)return;terminal=true;var updated=Math.max(0,Number(r.updated||0)),added=Math.max(0,Number(r.added||0)),total=Math.max(0,Number(r.total||0)),outside=Math.max(0,total-updated-added),s3=(r&&r.step3Timing)||{};el("spinner").style.display="none";el("message").className="box done";el("message").innerHTML="✓ 日次処理が完了しました。<div class=result><div class=group><div class=groupTitle>Search Console</div><div class=resultGrid><span>取得行</span><b>"+Number(r.rawRows||0).toLocaleString()+"件</b><span>有効な記事URL</span><b>"+Number(r.validRows||0).toLocaleString()+"件</b></div></div><div class=group><div class=groupTitle>記事DB</div><div class=resultGrid><span>更新記事</span><b>"+updated.toLocaleString()+"件</b><span>更新対象外</span><b>"+outside.toLocaleString()+"件</b><span>新規記事</span><b>"+added.toLocaleString()+"件</b><span>総記事数</span><b>"+total.toLocaleString()+"件</b></div><div class=groupNote>※更新対象外：今回のSearch Consoleデータ取得で対象とならなかった記事です。</div></div><div class=group><div class=groupTitle>改善効果</div><div class=resultGrid><span>モニタ中</span><b>"+Number(r.monitoringCount||0).toLocaleString()+"件</b></div></div><div class=group><div class=groupTitle>処理時間</div><div class=resultGrid><span>Search Console取得</span><b>"+formatTime(r.fetchElapsedSeconds)+"</b><span>分析・処理</span><b>"+formatTime(r.analysisElapsedSeconds)+"</b><span>STEP3・完了処理</span><b>"+formatTime(r.finalizeElapsedSeconds)+"</b><span>全体所要時間</span><b>"+formatTime(r.totalElapsedSeconds)+"</b></div></div><div class=group><div class=groupTitle>STEP3詳細</div><div class=resultGrid><span>履歴の確認（前）</span><b>"+formatTime(s3.beforeHistory)+"</b><span>改善の推移更新</span><b>"+formatTime(s3.effect)+"</b><span>履歴の確認（後）</span><b>"+formatTime(s3.afterHistory)+"</b><span>完了状態の保存</span><b>"+formatTime(Number(s3.markComplete||0)+Number(s3.runtime||0))+"</b><span>Home更新</span><b>"+formatTime(s3.home)+"</b><span>最終設定・後処理</span><b>"+formatTime(Number(s3.finalSettings||0)+Number(s3.cleanup||0)+Number(s3.summaryRead||0)+Number(s3.progress||0))+"</b></div><div class=groupNote>※STEP3の内訳は高速化箇所を特定するための品質保証用表示です。</div></div></div>";el("note").textContent="結果を確認して「閉じる」を押してください。";el("cancelBtn").textContent="閉じる";el("cancelBtn").style.display="inline-block";}'
    + '</script></body></html>';
...
function sbmBuildHomeSheet_() {
  ...
  var left = [['今日の改善','0件'],['モニター中','0件'],['未取得記事','0件'],['改善確認完了','0件'],['','']];
  for (var i=0;i<5;i++) {
    var r=15+i;
...
function sbmWriteArticleDbObjects_(map) {
  ...
  // 記事DBだけでなく、履歴・推移・Doctor系も同じGSC表記へ揃える。
  // GSCで今回確認できたURLだけを更新し、非取得記事の既存URLは変更しない。
...
function sbmAppendImprovementHistory_(data,row,before) {
  ...
  var record={
    ...
    '1週':'測定待ち','2週':'測定待ち','3週':'測定待ち','4週':'測定待ち','最終判定':'測定待ち','状態':'測定待ち',
...
function sbmLegacyHistoryObjects_(){
  ...
  vals.forEach(function(r){
    ...
    out.push({
      ...
      '1週':o['状態']==='完了'?'完了':'測定待ち','2週':'測定待ち','3週':'測定待ち','4週':'測定待ち','最終判定':o['状態']==='完了'?'完了':'測定待ち','状態':o['状態']==='完了'?'完了':'測定待ち','ArticleID':a['ArticleID']||'','記事URL':url,
...
function sbmMigrateLegacyMonitoringLabels_(){
  ...
  try{
    ...
    if(hs&&hs.getLastRow()>1){
      var hh=sbmHeaderMap_(hs), names=['1週','2週','3週','4週','最終判定','状態'];
      names.forEach(function(name){var c=hh[name];if(!c)return;var rg=hs.getRange(2,c,hs.getLastRow()-1,1),vals=rg.getValues(),changed=false;vals.forEach(function(r){if(String(r[0]||'').trim()==='未測定'){r[0]='測定待ち';changed=true;}});if(changed)rg.setValues(vals);});
    }
...
function sbmRefreshHome_() {
  ...
  sh.getRange('C17:D17').setValue(missingCount + '件');

  var weekly = sbmHomeWeeklyActivity_();
  sh.getRange('C18:D18').setValue(weekly.completed + '件');
...
function sbmDoctorRunHealthStageFromDialog(){
  ...
  try{
    if(run.statusCode==='PREPARING'){
      // Runner UI表示後の独立STEP。重い事前確認と180日API取得を同一実行に詰め込まない。
      ...
      run.statusCode='PREFLIGHT_DONE'; run.phase='事前確認完了'; run.nextStep='180日集計'; run.lastSuccessAt=sbmNowText_(); run.updatedAt=sbmNowText_(); sbmDoctorSaveHealthRun_(run);
      return sbmDoctorHealthDialogResult_(run,false,'開始準備が完了しました。180日集計へ進みます。');
    }
    if(run.statusCode==='PREVIOUS_DONE' || run.statusCode==='SCREENING'){
      var screening=sbmDoctorRunScreeningBatch_(true);
      run=sbmDoctorGetHealthRun_();
      return sbmDoctorHealthDialogResult_(run,!!screening.done,screening.message);
    }
...
function sbmDoctorRunScreeningBatch_(silent) {
  ...
  if(start>=current.length) return sbmDoctorFinalizeScreening_(silent,state);
  ...
  if(finish<current.length) return {done:false,message:'健康状態を '+finish+' / '+current.length+'件 判定しました。次の分割処理へ進みます。'};
  ...
  return {done:false,message:'全記事の判定が完了しました。次のSTEPで健康診断書を作成します。'};
}
...
function sbmDoctorShowCopyDialog_(payload, jsonText) {
  ...
  var html = '<!doctype html><html><head><base target="_top"><meta charset="UTF-8">' +
    ...
    'function registerWriterResult(){const result=document.getElementById("writerResult").value.trim();const st=document.getElementById("writerStatus");if(!result){st.className="status error";st.textContent="Writerの回答全文、または処置結果JSONを貼り付けてください。";return}const b=document.getElementById("writerRegisterButton");b.disabled=true;st.className="status";st.textContent="Writer処置結果を登録しています…";google.script.run.withSuccessHandler(function(r){b.disabled=false;if(!r||!r.ok){st.className="status error";st.textContent=r&&r.message?r.message:"処理に失敗しました。";return}st.className="status ok";st.textContent=r.message||"Writer処置結果を登録しました。"}).withFailureHandler(function(e){b.disabled=false;st.className="status error";st.textContent=e&&e.message?e.message:String(e)}).sbmDoctorRegisterWriterTreatmentResultFromDialog(result)}' +
    '</script></body></html>';
