...
function sbmDoctorStoreMergeTreatmentResult_(o){
  ...
  var raw=JSON.stringify(o),artifact=null;try{artifact=sbmArtifactStoreMergeResult_(o,m);}catch(eArtifact){throw new Error('Merge結果は受理しましたが、Driveへの成果物保存に失敗しました。処理を中止します。詳細：'+String(eArtifact&&eArtifact.message?eArtifact.message:eArtifact));}
  var existing=rec.hm['Merge結果JSON']?String(rec.values[rec.hm['Merge結果JSON']-1]||''):'';
  if(rec.hm['Merge結果JSON'])rec.values[rec.hm['Merge結果JSON']-1]=raw.length<=49000?raw:JSON.stringify({contract_name:'SIMS_MERGE_TREATMENT_RESULT_V1',case_id:m.caseId,treatment_result_id:m.resultId,artifact_url:artifact.resultFileUrl,merged_article_url:artifact.articleFileUrl,note:'Merge結果全文はDrive Artifactへ自動保存済み。セル上限のためSBMには参照情報を保存。'});
  ...
  sbmArtifactAttachToMergeCompletionContext_(rec,artifact);
  ...
  return {caseId:m.caseId,status:String(rec.values[rec.hm['状態']-1]||''),resultStatus:m.status,artifact:artifact,alreadyRegistered:!!(existing&&existing===raw)};
}
